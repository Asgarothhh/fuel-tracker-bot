...
**Дата генерации:** 2026-04-05 17:32 UTC
...
|---|---------|----------|-------------|----------------|-----|------|
| 1 | `belorusneft_parse` | Парсинг API / даты (Belorusneft) | 6 | 6 | 1.76 | **да** |
| 2 | `plates_import` | Номера, импорт API, дедупликация | 5 | 5 | 55.7 | **да** |
| 3 | `auth_permissions` | Права и токены привязки | 2 | 2 | 24.16 | **да** |
| 4 | `excel_ocr` | Схема чека и строка Excel | 2 | 2 | 7.71 | **да** |

### Цепочка (одна строка)

`belorusneft_parse` ✓ (1.76 ms) → `plates_import` ✓ (55.7 ms) → `auth_permissions` ✓ (24.16 ms) → `excel_ocr` ✓ (7.71 ms)

### Порядок выполнения

1. **belorusneft_parse** — успех (1.76 ms): _Парсинг API / даты (Belorusneft)_
2. **plates_import** — успех (55.7 ms): _Номера, импорт API, дедупликация_
3. **auth_permissions** — успех (24.16 ms): _Права и токены привязки_
4. **excel_ocr** — успех (7.71 ms): _Схема чека и строка Excel_
...
```text
  [старт]
      |
      v
  +-- belorusneft_parse  [OK]  1.76 ms
      |
      v
  +-- plates_import  [OK]  55.7 ms
      |
      v
  +-- auth_permissions  [OK]  24.16 ms
      |
      v
  +-- excel_ocr  [OK]  7.71 ms
      |
      v
  [конец]
```
...
flowchart LR
  N0["belorusneft_parse / OK / 1.76ms"]:::okNode
  N1["plates_import / OK / 55.7ms"]:::okNode
  N2["auth_permissions / OK / 24.16ms"]:::okNode
  N3["excel_ocr / OK / 7.71ms"]:::okNode
...
[
  {
    ...
    "date_time": "2026-04-05T17:30:16.958485",
    "imported_at": "2026-04-05T17:30:16.959568",
    "confirmed_at": null,
  ...
  },
  {
    ...
    "doc_number": "OCR-DEMO-001",
    "date_time": "2026-04-05T17:30:16.958485",
    "imported_at": "2026-04-05T17:30:16.959971",
    "confirmed_at": null,
    "exported_to_excel": false,
    "ready_for_waybill": false,
    "status": "new"
...
[
  {
    ...
    "created_by": null,
    "created_at": "2026-04-05T17:30:16.960479",
    "expires_at": "2026-04-05T17:30:16.958485",
    "status": "new",
    "telegram_id": null,
    "used_at": null,
    "note": null
...
```json
{
  ...
  "additional_info": "Иностранное общество с ограниченной ответственностью 'ЛУКОЙЛ Белоруссия'; УНП 100126124 РН СККО 119017536; Платежный документ; Кассир: Кобачук Н.Б.; УИ ЕАВВО009ЕТЕ405207181040",
